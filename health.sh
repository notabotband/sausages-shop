...
then [ sudo docker pull $CI_REGISTRY_IMAGE/sausage-backend:${VERSION} ] && [ sudo docker-compose stop green ] && [ sudo docker-compose --env-file /home/${DEV_USER}/.env_file up -d --scale green=3 blue=3 ] && [ sudo docker-compose stop blue ]
fi
